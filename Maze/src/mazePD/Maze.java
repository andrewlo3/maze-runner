...
public class Maze {
	...
	*/
	...
	public Cell getCellForDroid(DroidInterface droid)
	{
	...
	*/
	...
	public Coordinates[] getAdjCoords(DroidInterface droid) {
		int[][] offset= {{0,-1},{1,0},{0,1},{-1,0}};
		Coordinates currentCord = findDroid(droid).getCoordinates();
		Coordinates[] coordList = new Coordinates[4];
		...
		for (int i=0;i<4;i++)
		{
			...
			if (adjX >= 0 && adjX < getMazeDim() && adjY >= 0 && adjY < getMazeDim())
				coordList[i]= maze[adjX][adjY][currentCord.getZ()].getCoordinates();
			else
				coordList[i] = null;
		}
	...
	public Cell getCellDir(Cell currentCell, Direction direction) {
	...
	*/
	public Coordinates usePortal(DroidInterface droid)
	{
		Cell currentCell = getCellForDroid(droid);
